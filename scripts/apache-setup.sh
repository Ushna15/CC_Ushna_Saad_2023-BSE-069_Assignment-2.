#!/bin/bash
set -e

# Update and Install Apache
yum update -y
yum install httpd -y
systemctl start httpd
systemctl enable httpd

# Get metadata token (IMDSv2)
TOKEN=$(curl -s -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600")

# Get instance metadata
PRIVATE_IP=$(curl -s -H "X-aws-ec2-metadata-token: \$TOKEN" http://169.254.169.254/latest/meta-data/local-ipv4)
PUBLIC_IP=$(curl -s -H "X-aws-ec2-metadata-token: \$TOKEN" http://169.254.169.254/latest/meta-data/public-ipv4)
INSTANCE_ID=$(curl -s -H "X-aws-ec2-metadata-token: \$TOKEN" http://169.254.169.254/latest/meta-data/instance-id)

# Set hostname dynamically from Terraform variable
hostnamectl set-hostname ${server_name}

# Create custom HTML page
cat > /var/www/html/index.html <<EOF
<!DOCTYPE html>
<html>
<head>
    <title>Backend Server: ${server_name}</title>
    <style>
        body { font-family: Arial; margin: 50px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; }
        .container { background: rgba(255, 255, 255, 0.1); padding: 30px; border-radius: 10px; box-shadow: 0 8px 32px rgba(0,0,0,0.3); }
        .label { font-weight: bold; color: #ffd700; }
        .info { margin: 10px 0; padding: 10px; background: rgba(255,255,255,0.2); border-radius: 5px; }
    </style>
</head>
<body>
    <div class="container">
        <h1>🚀 Backend Web Server: ${server_name}</h1>
        <div class="info"><span class="label">Status:</span> ${server_status}</div>
        <div class="info"><span class="label">Instance ID:</span> \$INSTANCE_ID</div>
        <div class="info"><span class="label">Private IP:</span> \$PRIVATE_IP</div>
        <div class="info"><span class="label">Public IP:</span> \$PUBLIC_IP</div>
        <div class="info"><span class="label">Deployed On:</span> \$(date)</div>
        <div class="info"><span class="label">Managed By:</span> Terraform</div>
    </div>
</body>
</html>
EOF

chmod 644 /var/www/html/index.html